...
def run_test():
    ...
    def verify_no_errors_with_data(data, test_name):
        ...
        def totals():
            ...
            print(test_name + " total errors count: ", total_errors, " - every ", len(data) / total_errors, " OK: ",
                  len(data) - total_errors)
        ...
        print("")
        pat_to_print = {bin8(k1): v[1][:20], bin8(k): v1[1][:20], "noerr_big+": all_errors[:20],
                        "total": all_errors[:20]}
    ...
    amd_problem = 0x200
    amd_total = 0x8000
    amd_pat = b'\xee' * amd_problem + bytes(random.getrandbits(8) for i in range(0x100)) + b'\xaa' * (
                amd_total - 0x100 - amd_problem)
